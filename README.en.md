...
Only three endpoints. The structure keeps rules testable and decoupled from
Express/MongoDB. It is not meant to mimic a full enterprise system. Boundaries
exist only where they solve a concrete problem.
...
Create, delete, and tree retrieval each have their own use case. No catch-all
service; each rule is easy to find and test.

#### Repository as a port

Use cases depend on a contract, not Mongoose. Schemas, queries, and operators
stay in infrastructure; unit tests use in-memory fakes.

The port is not there for "multiple databases". It stops application logic from
depending on the persistence technology.

#### Public ID ≠ ObjectId

The challenge requires numeric IDs and `relatedId`. MongoDB still uses
`ObjectId` underneath; the API only exposes the numeric ID. The external
contract does not leak storage details.

#### Atomic counter

`max(id) + 1` races under concurrency. An atomic MongoDB counter keeps IDs unique
for parallel creates.

#### Adjacency list

Each item stores only its parent (`relatedId`). Creation stays simple, depth is
unbounded, and documents do not embed the whole tree.

#### In-memory tree, O(n)

One `find` returns a flat list; ID-keyed maps build the forest in linear time.
No per-level queries, no N+1, no Mongo-specific aggregation pipeline. Tree
assembly is a pure function, testable without infrastructure.

#### Subtree deletion

Deleting an item also removes descendants. The `ancestors` chain on each
document finds the subtree without recursive application queries.

#### Typed errors

Missing parent, missing item, duplicate name, broken hierarchy: dedicated types.
Infrastructure maps technical failures (e.g. unique index); HTTP only maps those
errors to status codes.

#### Validation at the edge

Zod checks shape and types on input. Parent existence and name uniqueness stay
in the use case.

#### Explicit composition

`main` wires repositories, generators, and use cases. No service locator, no
hidden `new` inside controllers.
...
Lint, types, tests, and docs checks in CI. They catch typing, boundary, OpenAPI,
HTTP contract, and MongoDB regressions.

#### Trade-offs

More files and concepts than routes + controllers + models. Upfront structure
costs more; the payoff is isolated rules, focused tests, visible dependencies,
and predictable errors.

For a throwaway API this would be overkill. For this challenge it is deliberate:
show organization, concurrency, testability, and hierarchy integrity.

#### What was not abstracted

No multi-database, multi-protocol, or out-of-scope features. The only ports are
real boundaries: HTTP, use cases, persistence, IDs, and tree assembly. Complexity
